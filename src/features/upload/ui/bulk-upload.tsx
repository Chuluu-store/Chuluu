...
export function BulkUpload({ groupId, onUploadComplete, onClose }: BulkUploadProps) {
  ...
  const handleFileSelect = useCallback(
    async (selectedFiles: FileList) => {
      ...
      if (errors.length > 0) {
        toast.error(`다음 파일들을 업로드할 수 없습니다:\n${errors.join('\n')}`);
      }
      ...
      if (totalFiles > 5000) {
        toast.error(`최대 5000개 파일까지만 업로드할 수 있습니다. (현재: ${totalFiles}개)`);
        return;
      }
  ...
  const startUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i += concurrentLimit) {
        ...
        console.log(
          `📦 Processing batch ${Math.floor(i / concurrentLimit) + 1}:`,
          batch.map((f) => f.file.name)
        );

        const batchPromises = batch.map((file) => uploadFileDirect(file));
  ...
  const uploadFileDirect = async (uploadFile: UploadFile) => {
    ...
    try {
      ...
      setFiles((prev) =>
        prev.map((f) =>
          f.id === uploadFile.id ? { ...f, status: 'completed', progress: 100, mediaId: result.mediaId } : f
        )
      );
  ...
  const overallProgress =
    uploadStats.total > 0 ? Math.round(((uploadStats.completed + uploadStats.failed) / uploadStats.total) * 100) : 0;
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6 bg-stone-800/50 backdrop-blur-xl border border-stone-700/50 rounded-3xl">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        {onClose && (
          <button onClick={onClose} className="p-2 hover:bg-stone-700/30 rounded-xl transition-colors">
      ...
      {/* 파일 드롭 영역 */}
      {files.length === 0 && (
        <FileDropZone onDrop={handleDrop} onDragOver={handleDragOver} onClick={() => fileInputRef.current?.click()} />
      )}
      ...
      {/* 파일 목록 */}
      <FileList files={files} isUploading={isUploading} onRemoveFile={removeFile} onRetryFile={retryFailedFile} />
    </div>
  );
}
