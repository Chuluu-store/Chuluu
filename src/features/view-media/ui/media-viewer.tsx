...
export function MediaViewer({ media, currentIndex, isOpen, onClose, onPrevious, onNext }: MediaViewerProps) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Close button */}
        <button onClick={onClose} className="absolute top-4 right-4 z-10 text-white hover:text-gray-300 p-2">
        ...
        <div className="max-w-full max-h-full flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
          {isVideo ? (
            <video src={currentMedia.path} controls autoPlay className="max-w-full max-h-full" />
          ) : (
            <Image
          ...
          {showInfo && (
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-300 mb-1">파일명</h4>
                  <p className="text-sm break-all">{currentMedia.originalName}</p>
                </div>

                <div>
                  <h4 className="text-sm font-medium text-gray-300 mb-1">크기</h4>
                  <p className="text-sm">{formatFileSize(currentMedia.size)}</p>
                </div>

                <div>
                  <h4 className="text-sm font-medium text-gray-300 mb-1">해상도</h4>
                  <p className="text-sm">
                ...
                {isVideo && currentMedia.metadata.duration && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-300 mb-1">재생시간</h4>
                    <p className="text-sm">{Math.round(currentMedia.metadata.duration)}초</p>
                  </div>
                )}

                <div>
                  <h4 className="text-sm font-medium text-gray-300 mb-1">업로드일</h4>
                  <p className="text-sm">{formatDate(currentMedia.createdAt)}</p>
                </div>

                {currentMedia.metadata.dateTaken && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-300 mb-1">촬영일</h4>
                    <p className="text-sm">{formatDate(currentMedia.metadata.dateTaken)}</p>
                  </div>
                )}

                {currentMedia.metadata.location && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-300 mb-1">위치</h4>
                    <p className="text-sm">
